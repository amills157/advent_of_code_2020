...
namespace advent_day_14 {
   class puzzle_14 {      
      ...
      public static  void partTwo(StreamWriter sw, ref  List <long> bitValues, ref IEnumerable<string> lines){
         ...
         foreach(string r in lines){
            ...
            if(splitStrings[0].Contains("mask")){
               ...
               List <char> memAddressMask = new List <char>();
               ...
               sw.WriteLine("Before - bitMask {0}", string.Join("", bitMask));

               sw.WriteLine("Before - memAddressMask {0}", string.Join("", memAddressMask));

               // I had this round the wrong way and was getting a close but not quite answer - Need to revisit this whol script and tidy it up / rewrite it
               ...
               sw.WriteLine("After - bitMask {0}", string.Join("", bitMask));

               sw.WriteLine("After - memAddressMask {0}", string.Join("", memAddressMask));                  

               foreach(string variation in GetVariations(string.Join("", memAddressMask)))
               {
      ...
      {
         // Load input
         IEnumerable<string> lines = File.ReadLines("puzzle_14_input.txt");
         ...
         {  
            ...
            partTwo(sw, ref bitValues, ref lines);
